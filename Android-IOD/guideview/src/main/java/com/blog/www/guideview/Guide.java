...
public class Guide implements View.OnKeyListener, View.OnClickListener {
  ...
  private MaskView onCreateView(Activity activity) {
    ...
    // parentY = loc[1];
    // }
    if(mConfiguration.viewOffsetY != 0 || mConfiguration.viewOffsetX !=0){
      parentX -= mConfiguration.viewOffsetX;
      parentY -= mConfiguration.viewOffsetY;
    }
    ...
    if (mConfiguration.mTargetView != null) {
      ...
      maskView.setTargetRect(Common.getViewAbsRectWithinputHeight(mConfiguration.mTargetView, parentX, parentY, 100));
    } else {
